...
public class MineriaService {
    ...
    /**
     * Metodo para regularizar a los usuarios que no se le ha asignado su rendimiento diario de mineria
     * este metodo tiene la facultad de editar la licencia de mineria y asignarle la ganancia actual correspondiente
     */
    public void regularizarRendimientoDiario() {
        List<LicenciaMineria> licenciasActivas = mineriaRepository.findByEstado(EstadoLicenciaMineria.ACTIVA);
        
        for(LicenciaMineria licencia : licenciasActivas) {

            //Obtiene los dias transcurridos desde la fecha de inicio de la licencia hasta la fecha actual
            long diasTranscurridos = java.time.Duration.between(licencia.getFechaInicio(), LocalDateTime.now()).toDays();
            //Calcula la ganancia actual correspondiente a los dias transcurridos
            double rDiaria = licencia.getTasaMineria().doubleValue() * licencia.getLicencia().getPrecio(); // Tasa diaria aproximada
            BigDecimal gananciaActual = new BigDecimal(rDiaria * diasTranscurridos);

            licencia.setGananciaActual(gananciaActual);
            mineriaRepository.save(licencia);
            System.out.println("Licencia ID: " + licencia.getId() + ", Ganancia Actual: " + licencia.getGananciaActual());
        }
    }
